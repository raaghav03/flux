...
const Navbar = () => {
    const [isMenuOpen, setMenuOpen] = useState(false) // Initialize state as null

    const toggleMenu = () => {
        setMenuOpen(prevState => (prevState === null ? true : !prevState));
    };
    ...
    return (
        <>
            ...
            <div className='p-4 flex justify-between w-full'>
                <Menu className={`md:hidden ${isMenuOpen !== null && (isMenuOpen ? 'block' : 'hidden')}`} />
                <div>
                    ...
                    <nav className="bg-white dark:bg-gray-900 fixed w-full z-20 top-0 start-0 border-b border-gray-200 dark:border-gray-600">
                        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                            ...
                            <div className="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
                                <PrimaryButton />
                                <button onClick={toggleMenu} type="button" className={`inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600 ${isMenuOpen !== null && isMenuOpen ? 'bg-gray-200' : ''}`} aria-controls="navbar-sticky" aria-expanded={isMenuOpen !== null ? isMenuOpen.toString() : undefined}>
                            ...
                            <div className={`items-center justify-between w-full md:flex md:w-auto md:order-1 ${isMenuOpen !== null && (isMenuOpen ? 'block' : 'hidden')}`} id="navbar-sticky">
